...
class LeNetReLUDropout(nn.Module):
    """
    LeNet implementation with ReLU activation and Dropout layers, adapted for various input sizes.
    """
    def __init__(self, num_classes=10, input_channels=1, dropout_rate=0.5):
        super(LeNetReLUDropout, self).__init__()
        self.features = nn.Sequential(
            # First convolutional layer: 6 feature maps, 5x5 kernel
            nn.Conv2d(input_channels, 6, kernel_size=5),  # Dynamic input channels
            # ReLU activation
            nn.ReLU(),
            # First pooling layer: 2x2 average pooling
            nn.AvgPool2d(2, stride=2),
            # Second convolutional layer: 16 feature maps, 5x5 kernel
...
def train_model(model, train_loader, test_loader, optimizer_type, learning_rate, 
    ...
    """
    Train the LeNet-ReLU-Dropout model
